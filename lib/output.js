...
function statistics(stats) {
  ...
  stats.forEach(element => {
    ret.push({
      label: element.title,
      value: element.displayValue,// title.indexOf('Bytes') === -1 ? stats[title] : prettyBytes(Number(stats[title]))
    });
  });
...
module.exports = (parameters, response) => {
  return Promise.resolve().then(() => {
    ...
    console.log(renderer(
      ...
      statistics([audits['first-contentful-paint'], audits['speed-index'], audits['interactive'], audits['first-meaningful-paint'], audits['first-cpu-idle'], audits['max-potential-fid']]),// statistics(response.pageStats),
